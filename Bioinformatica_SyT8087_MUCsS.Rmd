...
Esta es una actividad complementaria, pero altamente recoendable a seguir ya que nos introduce en el mundo de la bioinformática con códigos muy sencillos y usando herramientas disponibles para el análisis de secuencias que nos facilitarán mucho la vida.
...
Complementaria <- function(secuencia_dna = seq_dna){
  secuencia_dna = unlist(strsplit(x = secuencia_dna, split = "", fixed = TRUE))
...
Ahora bien, para facilitarnos la vida, usaremos el paquete de R llamado `bioseq`, que como su nombre nos indica es *una caja de herramientas para manipular secuencias biológicas* ([Keck, 2022](https://cran.r-project.org/web/packages/bioseq/index.html)). 
...
# 3.- Transcripción
Cuando vamos de camino a generar una proteína, necesitamos sacar la información desde dentro del núcleo al exterior y para ello usamos el ARN mensajero `ARNm`. Es decir, necesitamos **transcribir** la información desde la cadena de ADN a una cadena de ARN. Como ya sabréis las correspondencias en este caso cambian y tenemos que la *Timina* es reemplazada por el *Uracilo*, por lo que las parejas ahora son **Adenina-Uracilo* (**`A-U`**) y *Citosina-Guanina* (**`C-G`**). 

En términos de programación, lo único que tenemos que hacer es que 
